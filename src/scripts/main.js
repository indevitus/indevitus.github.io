...
(function () {
  const traverseAndAddClass = (parentClass, childClass) => {
    if (
      ...
      document.querySelectorAll(parentClass).length > 0
    ) {
  ...
  if (document.querySelector('.glide-team')) {
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      window.addEventListener('resize', (event) =>
        handleResize(event.target.innerWidth),
      );
    });
  }
  ...
  if (document.querySelector('.glide-testimonials')) {
    ...
    traverseAndAddClass(
      '.glide-testimonials .__glide__bullets',
      'glide__bullets',
    );
    traverseAndAddClass(
      '.glide-testimonials .__glide__bullet',
      'glide__bullet',
    );
    traverseAndAddClass(
      '.glide-testimonials .__glide__slides',
      'glide__slides',
    );
